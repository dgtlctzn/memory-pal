...
def relative_date(current_dt, event_dt, e_type, recurring):
    if e_type == 'Birthday' or e_type == 'Holiday' or (e_type == 'Other' and recurring):
    ...
    try:
        ...
        with connect(
            ...
            with cnx.cursor() as cursor:
                get_events_query = """
                SELECT CAST(date AS CHAR), date, type, name, id, recurring FROM Days
                ...
                for result in results:
                    date_str, dt, e_type, name, day_id, recurring = result
                    if e_type == 'Father\'s Day' or e_type == 'Mother\'s Day':
                        dt = get_next_parents_day(now, e_type)
                        date_str = dt.strftime('%Y-%m-%d %H:%M:%S')
                    days_away = relative_date(now, dt, e_type, recurring)
